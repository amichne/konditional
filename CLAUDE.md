...
│   └── src/main/kotlin/io/amichne/konditional/
├── kontracts/                                  # JSON Schema DSL (submodule)
├── opentelemetry/                  # OpenTelemetry observability hooks
...
```bash
# Build and test
./gradlew build                        # Full build with tests (all modules)
./gradlew :core:build                  # Build core module only
./gradlew test                         # Run tests only
./gradlew clean build                  # Clean build

# Code quality
./gradlew :core:detekt                 # Static analysis on core module
./gradlew :core:detektBaseline         # Generate baseline for existing issues

# Publishing
./gradlew :core:publishToMavenLocal    # Publish core to local Maven
...
| LLM docs | `llm-docs/` |
| Root build config | `build.gradle.kts`, `gradle.properties` |
| Core build config | `core/build.gradle.kts` |
...
- Tests mirror main package structure in `core/src/test/kotlin/`
...
4. **Adversarial tests** in `core/src/test/kotlin/io/amichne/konditional/adversarial/`
...
**Core Framework:**
- `core/src/main/kotlin/.../core/Namespace.kt` - Namespace abstraction and delegation
- `core/src/main/kotlin/.../core/FlagDefinition.kt` - Flag definition with rules
- `core/src/main/kotlin/.../core/evaluation/` - Evaluation logic
- `core/src/main/kotlin/.../serialization/SnapshotSerializer.kt` - JSON serialization

**Type System:**
- `core/src/main/kotlin/.../core/types/` - Value type system
- `core/src/main/kotlin/.../values/FeatureId.kt` - Feature identification

**Context:**
- `core/src/main/kotlin/.../context/Context.kt` - Context interface
- `core/src/main/kotlin/.../context/Platform.kt`, `AppLocale.kt` - Standard dimensions
- `core/src/main/kotlin/.../context/RampUp.kt` - Rollout bucketing
...
- `core/src/main/kotlin/.../internal/serialization/` - Moshi adapters and models

**Testing:**
- `core/src/test/kotlin/io/amichne/konditional/core/` - Core tests
- `core/src/test/kotlin/io/amichne/konditional/adversarial/` - Attack surface tests
...
**Recent Changes:**
- Multi-module structure: Core library moved to `core/` module
- `opentelemetry` module for OpenTelemetry observability hooks
...
- Module dependency updates: All modules now depend on `project(":core")` instead of root project
