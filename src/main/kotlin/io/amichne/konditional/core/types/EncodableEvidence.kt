 ...
 *
 * Supported types: Boolean, String, Int, Double, Enum<E>, JsonValue.JsonObject, JsonValue.JsonArray, CustomEncodeable
 *
...
sealed interface EncodableEvidence<T : Any> {
    ...
    companion object {
        ...
        inline fun <reified T : Any> get(): EncodableEvidence<T> {
            ...
            return when (T::class) {
                ...
                JsonValue.JsonArray::class -> JsonArrayEvidence as EncodableEvidence<T>
                    ...
                    // Check if T implements CustomEncodeable
                    else if (CustomEncodeable::class.java.isAssignableFrom(T::class.java)) {
                        CustomEvidence<T>() as EncodableEvidence<T>
                    } else {
                        throw IllegalArgumentException(
                            "Type ${T::class.qualifiedName} is not encodable. " +
                                "Supported types: Boolean, String, Int, Double, Enum, JsonObject, JsonArray, CustomEncodeable"
                        )
                    }
                }
            }
        ...
        inline fun <reified T : Any> isEncodable(): Boolean {
            return when (T::class) {
                ...
                else -> T::class.java.isEnum || CustomEncodeable::class.java.isAssignableFrom(T::class.java)
            }
        }
    }
     ...
     *
     * Typically used for data classes implementing [CustomEncodeable].
     */
