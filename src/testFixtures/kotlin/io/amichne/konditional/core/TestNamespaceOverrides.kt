...
/**
 * Extension functions for [Namespace] that provide convenient test-scoped override functionality.
 *
 ...
 *     ) {
 *         assertEquals(true, context.evaluate(TestFeatures.flagA))
 *         assertEquals("override", context.evaluate(TestFeatures.flagB))
 *         assertEquals(42, context.evaluate(TestFeatures.flagC))
 *     }
...
object TestNamespaceOverrides {
     ...
     * regardless of rules, context, or rollout configuration.
     *
    ...
    fun <R> Namespace.withOverrides(
        ...
        overrides.forEach { (feature, value) ->
            ...
            (registry as InMemoryNamespaceRegistry).setOverride(
        ...
        return try {
            ...
            overrides.forEach { (feature, _) ->
                @Suppress("UNCHECKED_CAST")
                (registry as InMemoryNamespaceRegistry).clearOverride(
                    feature as Feature<EncodableValue<Any>, Any, Context, *>
                )
            }
        }
