...
class AdversarialConfigTest {
    ...
    fun `rollout accepts exact boundary value 0_0 - users never see feature`() {
        // This compiles and is "valid" but means 0% rampUp
    ...
    fun `rollout accepts very small positive value - almost impossible to trigger`() {
        // Compiles fine but 0.001% rampUp is 1 in 100,000 users
...
//        assertEquals(Version(1, 0, 0), version)
    ...
    fun `multiple rules with same specificity - order determines winner`() {
        val TestNamespaceFeatures = object : Namespace.TestNamespaceFacade("adversarial-ambiguous-rules") {
        ...
        // Result depends on rule order and rampUp bucket
    ...
    fun `double values accept NaN - compiles but semantically invalid`() {
        ...
        val TestNamespaceFeatures = object : Namespace.TestNamespaceFacade("adversarial-nan") {
    ...
    fun `double values accept infinity - compiles but questionable semantics`() {
        ...
        val infinityDef = TestNamespaceFeatures.flag(TestNamespaceFeatures.infinityFlag)
    ...
    fun `int values at MAX_VALUE - potential overflow in calculations`() {
        ...
        val maxIntDef = TestNamespaceFeatures.flag(TestNamespaceFeatures.maxIntFlag)
    ...
    fun `string values can be empty - potentially invalid for some use cases`() {
        ...
        val emptyStringDef = TestNamespaceFeatures.flag(TestNamespaceFeatures.emptyStringFlag)
    ...
    fun `string values can be extremely long - no length validation`() {
        ...
        val TestNamespaceFeatures = object : Namespace.TestNamespaceFacade("adversarial-huge-string") {
            val hugeStringFlag by string<Context>(default = longString)
        }

        val hugeStringDef = TestNamespaceFeatures.flag(TestNamespaceFeatures.hugeStringFlag)
        assertEquals(1_000_000, hugeStringDef.defaultValue.length)
    }

    @Test
    fun `string values can contain special characters and newlines`() {
        val TestNamespaceFeatures = object : Namespace.TestNamespaceFacade("adversarial-special-chars") {
            val specialCharsFlag by string<Context>(
                default = "Line1\nLine2\tTab\u0000Null\r\nCRLF"
            )
        }

        val specialCharsDef = TestNamespaceFeatures.flag(TestNamespaceFeatures.specialCharsFlag)
    ...
    fun `salt defaults to v1 but can be any string`() {
        val TestNamespaceFeatures = object : Namespace.TestNamespaceFacade("adversarial-salt-defaults") {
        ...
        val defaultSaltDef = TestNamespaceFeatures.flag(TestNamespaceFeatures.defaultSaltFlag)
        val customSaltDef = TestNamespaceFeatures.flag(TestNamespaceFeatures.customSaltFlag)
    ...
    fun `salt can be empty string - affects bucketing determinism`() {
        val TestNamespaceFeatures = object : Namespace.TestNamespaceFacade("adversarial-salt-empty") {
            val emptySaltFlag by boolean<Context>(default = false) {
                salt("") // Empty salt compiles fine
            }
        }

        val emptySaltDef = TestNamespaceFeatures.flag(TestNamespaceFeatures.emptySaltFlag)
        assertEquals("", emptySaltDef.salt)
    }

    @Test
    fun `salt with special characters affects hash calculation`() {
        val TestNamespaceFeatures = object : Namespace.TestNamespaceFacade("adversarial-salt-special") {
        ...
        val specialSaltDef = TestNamespaceFeatures.flag(TestNamespaceFeatures.specialSaltFlag)
        assert(specialSaltDef.salt.contains(":"))
    }
    ...
    fun `multiple containers can use same feature key in different namespaces`() {
        val features1 = object : Namespace.TestNamespaceFacade("duplicate-key-a") {
            val duplicateKey by boolean<Context>(default = true)
        }

        val features2 = object : Namespace.TestNamespaceFacade("duplicate-key-b") {
        ...
        assertEquals(features1, features1.duplicateKey.namespace)
        assertEquals(features2, features2.duplicateKey.namespace)
        val def1 = features1.flag(features1.duplicateKey)
        val def2 = features2.flag(features2.duplicateKey)
    ...
    fun `features register eagerly - container initialization triggers registration`() {
        val container = object : Namespace.TestNamespaceFacade("eager-registration") {
    ...
    fun `inactive flag always returns default regardless of rules`() {
        ...
        // Even though rule matches and rampUp is 100%, flag is inactive
        assertEquals(false, featureContainer.inactiveFlag.evaluate(context))
        val inactiveDef = featureContainer.flag(featureContainer.inactiveFlag)
    ...
    fun `rule matches but rollout excludes - falls through to next rule or default`() {
        ...
        // First rule matches platform but rampUp is 0%, so skip to second rule
        // Second rule matches and has 100% rampUp
