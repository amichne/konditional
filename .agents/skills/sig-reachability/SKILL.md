...
```bash
python3 "$CLAUDE_PROJECT_DIR/.Codex/hooks/sig_reachability.py" \
...
python3 "$CLAUDE_PROJECT_DIR/.Codex/hooks/sig_reachability.py" \
  --sig-dir "$CLAUDE_PROJECT_DIR/.signatures" \
  --report-file "$CLAUDE_PROJECT_DIR/.Codex/sig-reachability-report.html"

open "$CLAUDE_PROJECT_DIR/.Codex/sig-reachability-report.html"
```

## Suppressing known noise — exclusions file

Edit `.Codex/sig-reachability-exclusions.txt` to silence entries you know are
...
the exclusions file the moment Codex edits that file, so the next report
