...
rule to match; they only bypass the
rollout gate.
...
`id` used for rule matching and snapshot
serialization.
...
and reproducible. Konditional derives the
bucket from `SHA-256("$salt:$flagKey:${stableIdHex}")` and reduces it into `[0, 10_000)`.

Learn
more: [Deterministic ramp-up bucketing](/how-to-guides/rolling-out-gradually), [Theory: Determinism Proofs](/theory/determinism-proofs)
...
The runtime inputs to evaluation (for example: locale, platform, app version, and stable ID). Rules match against
...
- **Mechanism**: Stable rule precedence (specificity ordering + tie-break by definition order) and deterministic rollout
  bucketing.
...
A JSON encoding pattern for polymorphic types (like Kotlin sealed classes) where a `type` field identifies which variant
is being represented. In OpenAPI
schemas, discriminators map type values to schema references for proper client codegen.

Examples in Konditional: `FlagValue` (BOOLEAN/STRING/INT/DOUBLE/ENUM/DATA_CLASS) and `VersionRange` (
...
targeting beyond built-in criteria (
...
A typed configuration value (boolean/string/int/enum/custom) declared as a delegated property on a `Namespace`. Features
...
that namespace (without changing feature
definitions).
...
An isolation boundary with its own registry and independent configuration lifecycle (load/rollback/disable). Namespaces
...
percentage only adds users (for a
...
Restoring a prior configuration snapshot from a bounded history maintained by the registry. Rollbacks are
namespace-scoped and revert the active configuration
...
passes rollout/allowlist gates, the rule's
value is returned.
...
for that feature (useful when re-running
experiments).
...
Evaluating a feature against a baseline registry (returned value) while also evaluating against a candidate registry for
comparison telemetry. This supports
migrations by surfacing mismatches without changing behavior.

Learn
more: [Shadow evaluation](/observability/shadow-evaluation), [Theory: Migration and Shadowing](/theory/migration-and-shadowing)

## Snapshot

A serialized JSON representation of configuration state used as a storage/transport format. Snapshots are parsed at a
validated trust boundary before they can
...
A rule precedence metric: more specific rules are evaluated first. In Konditional, total specificity is the sum of base
targeting criteria (
...
A stable identifier used for deterministic bucketing and allowlists. `StableId.of(input)` normalizes a non-blank string
...
A version constraint used in rules (min/max/unbounded) to target contexts by semantic app version. Version ranges
participate in rule matching and contribute to
