...
val generateOpenApiSpec = tasks.register<GenerateOpenApiSpecTask>("generateOpenApiSpec") {
    group = "documentation"
    description = "Generate contract-first OpenAPI spec for the config metadata surface."

    generatorMainClass.set("io.amichne.konditional.configmetadata.contract.openapi.SurfaceOpenApiSpecGenerator")
...
val openapiSpecElements = configurations.create("openapiSpecElements") {
    ...
    description = "OpenAPI specification artifact for config metadata surface contracts."
...
tasks.named<Test>("test") {
    ...
    doFirst {
        systemProperty(
            "io.amichne.konditional.configmetadata.generatedOpenApiPath",
