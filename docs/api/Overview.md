...
```kotlin
// Create a Feature for Boolean values
val DARK_MODE: Feature<EncodableValue.BooleanEncodeable, Boolean, Context> =
    Feature("dark_mode")

// Or use factory methods for specific types
val API_ENDPOINT: Feature.OfJsonObject<ApiConfig, Context> =
    Feature.jsonObject("api_endpoint")
```
