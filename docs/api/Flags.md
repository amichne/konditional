...
```kotlin
// Production features
enum class ProdFeatures(override val key: String) : Feature<...> {
    NEW_CHECKOUT("prod_checkout_v2");

    override val registry: FlagRegistry = FlagRegistry
}

// Experimental features
enum class ExpFeatures(override val key: String) : Feature<...> {
    ML_RECOMMENDATIONS("exp_ml_recommendations");

    override val registry: FlagRegistry = FlagRegistry
}
```

### Avoid Feature Proliferation

Keep the number of features manageable. Remove obsolete features:
...
enum class Features(override val key: String) : Feature<...> {
    ...
    // TODO: Remove after 100% rollout (ETA: 2024-Q2)
    @Deprecated("Rolled out to 100%, remove in next release")
