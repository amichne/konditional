...
DOC_TARGET_ROOT = Path("docusaurus/docs/signature-targets")
...
def parse_markdown_doc(doc_path: Path) -> tuple[set[str], set[str]]:
    ...
    if lines and lines[0].strip() == "---":
        for index in range(1, len(lines)):
            ...
            if ":" in raw_line:
                ...
                if FRONTMATTER_KEY_PATTERN.match(candidate):
                    frontmatter_fields.add(candidate)
    ...
    for raw_line in lines[content_start:]:
        ...
        headings.add(canonical_heading(heading))

    return frontmatter_fields, headings


def expected_doc_ref_for_target(target: TargetRecord) -> str:
    if target.kind == "package":
        package_path = "_default" if target.package == "<default>" else target.package.replace(".", "/")
        relative = DOC_TARGET_ROOT / "package" / package_path
        return f"{relative.as_posix()}.md"

    if target.kind == "type":
        fqcn = target.target_id.removeprefix("type:")
        relative = DOC_TARGET_ROOT / "type" / fqcn.replace(".", "/")
        return f"{relative.as_posix()}.md"

    entrypoint = target.target_id.removeprefix("entrypoint:")
    relative = DOC_TARGET_ROOT / "entrypoint" / entrypoint
    return f"{relative.as_posix()}.md"


def build_doc_target_manifest(targets: list[TargetRecord]) -> dict[str, Any]:
    entries = []
        ...
        entries.append(
            {
                ...
                "kind": target.kind,
                "expected_doc_ref": expected_doc_ref_for_target(target),
    ...
    return {
        "version": 1,
        "doc_root": DOC_TARGET_ROOT.as_posix(),
        "entries": entries,
    }


def doc_manifest_index(doc_manifest: dict[str, Any]) -> dict[str, str]:
    mapping: dict[str, str] = {}
    for entry in doc_manifest.get("entries", []):
        target_id = entry.get("target_id")
        expected_doc_ref = entry.get("expected_doc_ref")
        if isinstance(target_id, str) and isinstance(expected_doc_ref, str):
            mapping[target_id] = expected_doc_ref
    return mapping
...
def validate_doc_structure(
    repo_root: Path,
    target: TargetRecord,
    expected_doc_ref: str,
    schema_target_types: dict[str, Any],
    doc_parse_cache: dict[str, tuple[set[str], set[str]]],
) -> list[str]:
    doc_path = repo_root / expected_doc_ref
    if not doc_path.exists():
        return []

    required = schema_target_types.get(target.kind, {})
    required_sections = [canonical_heading(value) for value in required.get("required_sections", [])]
    required_fields = required.get("required_fields", [])

    if expected_doc_ref not in doc_parse_cache:
        doc_parse_cache[expected_doc_ref] = parse_markdown_doc(doc_path)
    frontmatter_fields, headings = doc_parse_cache[expected_doc_ref]
...
def validate_claims(
    ...
    symbols: dict[str, SymbolRecord],
    expected_doc_refs: dict[str, str],
    ...
    for claim in claims:
        ...
        expected_doc_ref = expected_doc_refs.get(target_id)
        if expected_doc_ref is None:
            invalid_by_target["<unscoped>"].append(
                f"line {line_no}: target_id={target_id} has no deterministic doc mapping"
            )
            continue
        ...
        if not isinstance(doc_ref, str) or not doc_ref:
            ...
            continue
        if doc_ref != expected_doc_ref:
            invalid_by_target[target_id].append(
                f"line {line_no}: doc_ref must match deterministic mapping ({expected_doc_ref})"
            )
            continue
...
def build_coverage_report(
    ...
    for target in targets:
        ...
        expected_doc_ref = expected_doc_refs.get(target.target_id, "")
        doc_exists = bool(expected_doc_ref) and (repo_root / expected_doc_ref).exists()
        if doc_exists:
            docs_present += 1
        else:
            docs_missing += 1
        ...
        if target_invalid:
            ...
            status = "stale"
            stale += 1
        elif target_valid and doc_exists:
            status = "complete"
            complete += 1
        else:
            status = "missing"
        ...
        entries.append(
            {
                ...
                "scope_sig_paths": list(target.scope_sig_paths),
                "target_hash": target.target_hash,
                "expected_doc_ref": expected_doc_ref,
...
def normalize_claim_for_assembly(claim: dict[str, Any]) -> dict[str, Any] | None:
    if claim.get("_invalid"):
        return None
...
def build_doc_collation_bundle(
    targets: list[TargetRecord],
    symbols: dict[str, SymbolRecord],
    claims: list[dict[str, Any]],
    ...
    expected_doc_refs: dict[str, str],
    docs_dir: Path,
    surface_hash: dict[str, Any],
) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    coverage_entries = coverage.get("targets", [])
    coverage_by_target = {
        entry["target_id"]: entry
        for entry in coverage_entries
        if isinstance(entry, dict) and isinstance(entry.get("target_id"), str)
    }
    coverage_unscoped = coverage.get("unscoped_issues", [])

    claims_by_target: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for claim in claims:
        normalized = normalize_claim_for_assembly(claim)
        if normalized is None:
            continue
        target_id = claim.get("target_id")
        if not isinstance(target_id, str):
            continue
        claims_by_target[target_id].append(normalized)

    for target_id in claims_by_target:
        claims_by_target[target_id].sort(key=lambda item: (item["claim_id"], item["doc_ref"], item["target_hash"]))

    records: list[dict[str, Any]] = []
    sorted_targets = sorted(targets, key=lambda item: (item.kind, item.target_id))
    for target in sorted_targets:
        coverage_entry = coverage_by_target.get(target.target_id, {})
        coverage_status = coverage_entry.get("status", "missing")
        coverage_issues = list(coverage_entry.get("issues", []))
        blocking_reasons = sorted(dict.fromkeys(coverage_issues))
        if coverage_status in {"invalid", "stale"} and not blocking_reasons:
            blocking_reasons = [f"coverage status is {coverage_status}"]

        symbol_declarations = [
            {
                "symbol_id": symbol_id,
                "declaration": symbols[symbol_id].declaration,
            }
            for symbol_id in target.symbol_ids
            if symbol_id in symbols
        ]
        expected_doc_ref = expected_doc_refs.get(target.target_id, "")
        ready_for_write = coverage_status == "missing" and len(blocking_reasons) == 0
        ...
        records.append(
            {
                ...
                "claims": claims_by_target.get(target.target_id, []),
    ...
    queue_records = [
        {
            ...
            "kind": record["kind"],
            "expected_doc_ref": record["expected_doc_ref"],
            "target_hash": record["target_hash"],
            "coverage_status": record["coverage_status"],
            "symbol_ids": record["symbol_ids"],
            "symbol_declarations": record["symbol_declarations"],
        }
        for record in records
        if record["ready_for_write"]
    ]
    queue_records.sort(
        key=lambda item: (
            COVERAGE_PRIORITY.get(item["coverage_status"], 999),
            item["kind"],
            item["target_id"],
        )
    )
    ...
    blockers_targets = [
        ...
        for record in records
        if len(record["blocking_reasons"]) > 0
    ]
    blockers_targets.sort(key=lambda item: (item["kind"], item["target_id"]))

    assembly = {
        "version": 1,
        "source_artifacts": {
            "doc_target_manifest": (docs_dir / "doc-target-manifest.json").as_posix(),
            "targets": (docs_dir / "targets.json").as_posix(),
            "claims_ledger": (docs_dir / "claims-ledger.jsonl").as_posix(),
            "coverage_report": (docs_dir / "coverage-report.json").as_posix(),
            "surface_hash": (docs_dir / "surface-hash.json").as_posix(),
            "surface_global_hash": surface_hash.get("global_hash", ""),
        },
        "records": records,
    }
    queue = {
        "version": 1,
        "order": "coverage_status(missing-first),kind,target_id",
        "batch_hint": {
            "min": 1,
            "max": 3,
        },
        "total_ready": len(queue_records),
        "queue": queue_records,
    }
    blockers = {
        "version": 1,
        "summary": {
            "blocked_targets": len(blockers_targets),
            "unscoped_issues": len(coverage_unscoped),
        },
        "targets": blockers_targets,
        "unscoped_issues": coverage_unscoped,
    }
    return assembly, queue, blockers
...
def run_pipeline(repo_root: Path, mode: str, fail_on_missing: bool) -> None:
    ...
    doc_manifest_path = docs_dir / "doc-target-manifest.json"
    schemas_path = docs_dir / "schemas.json"
    claims_path = docs_dir / "claims-ledger.jsonl"
    coverage_path = docs_dir / "coverage-report.json"
    doc_assembly_path = docs_dir / "doc-assembly.json"
    doc_write_queue_path = docs_dir / "doc-write-queue.json"
    doc_blockers_path = docs_dir / "doc-blockers.json"
    ...
    doc_manifest = build_doc_target_manifest(targets)
    expected_doc_refs = doc_manifest_index(doc_manifest)

    write_json(targets_path, build_targets_payload(targets, symbols))
    write_json(doc_manifest_path, doc_manifest)
    write_json(schemas_path, schemas)
    write_determinism_trace(trace_path, traces)
    surface_hash = build_surface_hash(signatures_dir, index_paths, targets)
    write_json(surface_hash_path, surface_hash)
    ensure_claims_ledger(claims_path)

    doc_structure_issues: dict[str, list[str]] = defaultdict(list)
    doc_parse_cache: dict[str, tuple[set[str], set[str]]] = {}
    for target in targets:
        expected_doc_ref = expected_doc_refs.get(target.target_id)
        if expected_doc_ref is None:
            doc_structure_issues["<unscoped>"].append(
                f"target_id={target.target_id} has no deterministic doc mapping"
            )
            continue
        for issue in validate_doc_structure(
            repo_root=repo_root,
            target=target,
            expected_doc_ref=expected_doc_ref,
            schema_target_types=schemas["target_types"],
            doc_parse_cache=doc_parse_cache,
        ):
            doc_structure_issues[target.target_id].append(
                f"doc {expected_doc_ref}: {issue}"
            )
    ...
    valid_claims, invalid_by_target, stale_by_target = validate_claims(
        ...
        symbols=symbols,
        expected_doc_refs=expected_doc_refs,
    )
    for target_id, issues in doc_structure_issues.items():
        invalid_by_target[target_id].extend(issues)
    ...
    assembly, write_queue, blockers = build_doc_collation_bundle(
        targets=targets,
        symbols=symbols,
        claims=claims,
        coverage=coverage,
    ...
    write_json(doc_assembly_path, assembly)
    write_json(doc_write_queue_path, write_queue)
    write_json(doc_blockers_path, blockers)
